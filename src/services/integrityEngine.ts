 ...
 * Ported to TypeScript for stability and deployment.
 */
 ...
 */
export const checkLpEligibility = async (fairScoreTier: number, walletEvents: any[]): Promise<{ eligible: boolean, reason?: string, gini: number }> => {
  ...
  const balances = recentEvents.reduce((acc: any, event: any) => {
 ...
 */
export const calculateVoterWeight = (score: number, hhi: number = 0): number => {
